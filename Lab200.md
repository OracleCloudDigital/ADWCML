...
MEDIUM 데이터베이스 서비스는 쿼리에 대해 여러 컴퓨팅 및 IO 리소스를 제공합니다. 또한이 서비스는 HIGH 데이터베이스 서비스와 비교하여 더 많은 동시성(concurrency - 실행관점)을 제공합니다.
...
## 주문 내역에 대한 쿼리 실행
주문 내역은 60억개의 행을 가진 데이터로 위에서 다뤘던 현재의 데이터보다 60배 가량 더 큽니다. 이 데이터에 일시적으로 엑세스 권한을 주고 쿼리를 하는 비즈니스 케이스는 무엇이 있을까요? 예를 들어 장기간에 걸친 패턴을 밝히기 위한 용도로 Data Scientist 들이 요청할 수 있겠네요. 이들은 과거에 데이터를 추출하고 데이터 저처리에 수 주일이 걸린 소규모 그룹입니다. 그들은 실시간 데이터에 즉시 엑세스하고 몇 분안에 결과를 얻을 수 있습니다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExNDcwMzgyODddfQ==
-->